...
app/
  ...
  crm/page.tsx         → CRM: tablero Kanban, estadísticas y ficha de oportunidad
  layout.tsx / globals.css
components/
  ContactForm.tsx      → formulario público (crea cliente + oportunidad)
  KanbanBoard.tsx       → tablero por estados con drag & drop
  LeadCard.tsx          → tarjeta de una oportunidad
  OpportunityModal.tsx  → ficha completa: datos, cambio de estado, tareas y notas
...
El esquema (`supabase/schema.sql`) tiene 6 tablas:

**En uso activo por la interfaz:**
- `clientes` — particulares o empresas que piden presupuesto
- `oportunidades` — el pipeline de negocio (lo que ves en el Kanban), enlazada a un cliente
- `tareas` — seguimiento comercial por oportunidad
- `notas` — historial de interacciones (llamadas, emails, reuniones)

**Preparadas para el futuro (sin pantalla propia todavía, pero ya en la base de datos):**
- `usuarios` — comerciales del equipo. `oportunidades.comercial_id` y `tareas.usuario_id` ya apuntan aquí (en `null` mientras no haya login), listas para cuando añadas Supabase Auth y quieras asignar quién lleva cada oportunidad.
...
- Login de comerciales con Supabase Auth y una pantalla para asignar oportunidades/tareas (las columnas `comercial_id` y `usuario_id` ya existen).
...
- Filtros y buscador en el tablero (por fecha, tipo de evento, rango de presupuesto).
